...
# Public Cmdlets
###############################################################################
function Set-ArtifactReference
{
<#
.SYNOPSIS
...
#>
    ...
    begin
    {
        ...
        function ProcessSQLProjectFile([string] $file)
        {
            ...
            [HashTable]$mainPropertiesHash = $CustomConfigurationProperties
          

            # Are there any main properties to change?
            if ($mainPropertiesHash.Count -gt 0)
            {
               HandleArtifactReferences -file $file -fileXML $fileXML -newMainProps $mainPropertiesHash
            }
        ...
        function ProcessProjectFile([string] $file)
        {
            ...
            switch -Wildcard ($ext)
            {
                "*.sqlproj"
                {
                    ProcessSQLProjectFile -file $file
                }
